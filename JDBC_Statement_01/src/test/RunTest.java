...
public class RunTest {
	...
	public static void main(String[] args) {
		 ...
		 * -[Prepared]Statement : 연결된 데이터베이스(DB)에 SQL문을 전달해서 실행하고, 결과를 받아내는 객체
		 ...
		 * 5) 결과 받기
		 * 		> SELECT문 객체 => ResultSet객체 (조회된 데이터들이 담겨있음) => 6_1)
		 * 		> 	 DML문 실행 => int(처리된 행 수)
		 * 
		...
		//앞으로 실행할 sql문("완성형태"로 만들어 두기)(맨 뒤에 세미콜론(;)는 없어야된다.!)
		...
		//1)jdbc driver 등록
		try {
			Class.forName("oracle.jdbc.driver.OracleDriver"); //ojdbc6.jar 파일을 추가 안했을 경우 || 추가는했는데 오타가 있을 경우 => ClassNotFoundException 발생
			//System.out.println("OracleDriver 등록성공");
			
			//2) Connection 객체 생성 : DB에 연결(url, 계쩡명, 비밀번호)
			conn = DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe","JDBC","JDBC");
			
			//3) Statement 객체 생성
			stmt = conn.createStatement();
			
			//4, 5) sql문 전달하면서 실행 후 결과 받기(처리된 함수)
